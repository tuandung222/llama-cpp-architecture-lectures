...
GGML định nghĩa interface `ggml_backend` (trong `ggml/include/ggml-backend.h`) mà mọi hardware backend phải implement:

```c
struct ggml_backend {
    const char * name;

    // Quản lý bộ nhớ trên device
    ggml_backend_buffer_t (*alloc_buffer)(ggml_backend_t, size_t);

    // Thực thi computation graph trên device
    ...
    // Synchronize (đợi GPU hoàn thành)
...
CUDA backend (`ggml/src/ggml-cuda/`, 93 files) hỗ trợ NVIDIA GPU:
